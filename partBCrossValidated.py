...
'''
Next make 3 version of neural network with actication functions
Sigmoid
RELU
Leaky RELU

Each model is tuned for learning rate and crossvalidated.
'''
...
for i, (train_index, test_index) in  enumerate(kf.split(x, y)):
    for j, eta in enumerate(etas):
        ...
        scores = NN.fit(x[train_index], y[train_index], Adam(eta, 0.9, 0.99), batches=batches, epochs=epochs, X_val = x[test_index], t_val = y[test_index])
...
MSE_method.append(np.min(np.mean(MSEs, axis=0)))
method_names.append(method_name)
print(f"\n Average MSE test Sigmoid is {np.mean(MSEs, axis=0)}")
...
MSE_method.append(np.min(np.mean(MSEs, axis=0)))
method_names.append(method_name)
print(f"\n Average MSE test RELU is {np.mean(MSEs, axis=0)}")
...
MSE_method.append(np.min(np.mean(MSEs, axis=0)))
method_names.append(method_name)
print(f"\n Average MSE test LRELU is {np.mean(MSEs, axis=0)}")


#Do a regular OLS for benchmarking the neural networks
...
print(f"\n The methods tested where: \n {method_names}")
print(f"\n The best avg MSE for these methods were: \n {np.round(MSE_method, decimals = 5)}")
